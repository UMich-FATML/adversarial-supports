...
_CIFAR_FS_DATASET_DIR = '<INSERT_CIFAR_FS_PATH_HERE>'
...
class FewShotDataloader():
    def __init__(self,
                 ...
                 fix_categories=None        # Fix categories to sample classes from. Shape: (batch_size, nKnovel)
                 ):
        ...
        assert fix_categories is None or \
            (fix_categories.shape[0] == batch_size and fix_categories.shape[1] == nKnovel)
        self.fix_categories = fix_categories
    ...
    def sample_episode(self, iter_idx):
        ...
        if self.fix_categories is None:
            ...
            # fix_categories is set only for experimental purposes
            # it's set on with nKbase = 0, thus no base categories is set
            # nKnovel = train_shot and it returned through fix categories
            idx = iter_idx % self.batch_size
            Kbase = []
            Knovel = sorted(list(self.fix_categories[idx]))
    ...
    def get_iterator(self, epoch=0):
        ...
        def load_function(iter_idx):
            Exemplars, Test, Kall, nKbase = self.sample_episode(iter_idx)
        ...
        data_loader = tnt_dataset.parallel(
            ...
            shuffle=(False if (self.is_eval_mode or self.fix_categories is not None) else True))
...
if __name__ == "__main__":
    ...
    ds = CIFAR_FS(phase='train')
    # novelcats = random.sample(list(ds.labelIds_novel), 5)
